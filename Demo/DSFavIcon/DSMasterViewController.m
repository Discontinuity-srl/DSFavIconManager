...
- (void)viewDidLoad
{
    ...
    [[self topDomains] enumerateObjectsUsingBlock:^(NSString* domain, NSUInteger idx, BOOL *stop) {
...
- (void)updateHeaderLabel {
    ...
    self.headerLabel.text = [NSString stringWithFormat:@"Missing Icons: %d/%d (%d disabled)", _domainsWithoutIcon.count, _objects.count, [self domainsToSkip].count];
}
...
- (BOOL)URLWantsIcon:(NSURL*)url {
    return ![[self domainsToSkip] containsObject:[url absoluteString]];
}
